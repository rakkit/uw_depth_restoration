...
class estimation(nn.Module):
    ...
    def forward(self, x):
        ...
        trans = self.OutConv(self.up(self.convt(trans)))
...
class DepthNet(nn.Module):
    ...
    def forward(self, x):
        ...
        trans = self.conv1X(x)
        features = torch.mul(trans, xmin)
        trans = self.OutConv(self.up(self.convt(features)))
        trans = torch.sigmoid(trans) + 1e-12        
        return features, 1/(10*trans+0.05)
        # return 1/trans
...
class BetaDecoder(nn.Module):
    ...
    def forward(self, x, depth):
        ...
        out = out.view(-1, 3, 4)
        #################
        # ! THIS IS VERYYYYYYYYY IMPORTANT!
        # pose = out[:, :, 0:1] * torch.exp((out[:, :, 1:2] * copy_depth)) + out[:, :, 2:3] * torch.exp((out[:, :, 3:4] * copy_depth))
